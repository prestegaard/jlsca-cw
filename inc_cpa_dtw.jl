...
if FFT_ENABLED == 1
    ...
    reference = getSamples(trs, 1)
    radius = 15
    addSamplePass(trs, x -> (path=fastdtw(reference,x,radius); y=align(x,path); y))
...
plot(PlotScoresEvolution(),rankData,params, 1,1, show=true)
